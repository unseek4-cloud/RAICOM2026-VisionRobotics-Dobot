...
同时现场确认 `robot.motion.place_inspection_z_mm`：当前按实测限制为 `410 mm`。吸取后先在原抓取 XY 竖直升到该高度，再保持 Z 不变只移动 XY 到观察位；该值既要实际可达，也要使放置点处于 D435 有效量程。任务三放置测高单独使用 `tasks.task3.placement_vision.depth_min_mm=250`，以识别距离更近的后续堆顶；抓取区的 `camera.depth_min_mm=300` 不变。任务三视觉失败时程序会保持吸盘并停止，必须按现场安全流程人工处置，不能填写固定放置高度兜底。
