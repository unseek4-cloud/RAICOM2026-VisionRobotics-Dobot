...
- `expected_objects/max_objects`：任务二默认 2，任务三评分默认 2；现场任务书优先。达到 `max_objects` 后还要重新观察并连续确认“当前任务没有额外目标”；若仍有同任务目标，应报数量不符而不是静默切换任务。
...
- 当前任务“无目标”必须连续多帧确认，并结合 `expected_objects/max_objects`；达到上限后发现额外同任务目标必须失败告警；
